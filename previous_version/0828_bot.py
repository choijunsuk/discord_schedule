...
# 나티악 이벤트 횟수 저장용
natiak_count = {'오메가': 0, '로얄': 0, '로얄': 0}
natiak_order = ['혈맹','오메가' ,'로얄',]  # 나티악 패턴 순서
...
def initialize_schedule(natiak_start_time=None, natiak_start_event=None, natiak_start_count=None):
    ...
    if natiak_start_time and natiak_start_event is not None and natiak_start_count is not None:
        # 기존 나티악 이벤트 중 나티악 시작 시간 이후의 이벤트를 제거
        schedule_data = [
            event for event in schedule_data if not event['event'].startswith('나티악') or event['datetime'] < natiak_start_time
        ]
        ...
        while True:
            ...
            # 7일(168시간) 뒤의 이벤트까지만 생성 (필요에 따라 조정 가능)
            if current_time > natiak_start_time + timedelta(hours=168):
                break
    else:
        # 기존 방식으로 스케줄 생성
        schedule_data = []
        natiak_count = {'오메가': 0, '로얄': 0, '혈맹': 0}
        natiak_index = 0
        for i in range(7):
